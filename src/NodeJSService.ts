...
export default class LightningServiceNodeJS extends LocalMain.LightningService {
	...
	async finalizeNewSite (): Promise<void> {
		...
		try {
			...
			// The Atlas Ecommerce template requires two extra plugins.
			if (this._site.customOptions?.bpId === 'ecommerce-blueprint-atlas-1') {
				/**
				 * Install Atlas Commerce Blocks.
				 * See https://github.com/wpengine/atlas-commerce-blocks.
				 */
				await wpCli.run(this._site, [
					'plugin',
					'install',
				...
				/**
				 * Install Atlas Commerce Connector for BigCommerce.
				 * See https://github.com/wpengine/atlas-commerce-connector-bigcommerce.
				 */
				await wpCli.run(this._site, [
					'plugin',
					'install',
